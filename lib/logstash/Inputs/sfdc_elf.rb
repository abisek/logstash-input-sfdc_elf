...
class LogStash::Inputs::SfdcElf < LogStash::Inputs::Base
  ...
  # The second part of logstash pipeline is run, where it expects to have event
  # objects generated and passed into the queue.
  ...
  def run(queue)
    ...
    loop do
      ...
      soql_expr = "SELECT Id, EventType, Logfile, LogDate, LogFileLength
                   FROM EventLogFile
                   WHERE LogDate > #{@last_read_log_date} ORDER BY LogDate DESC "
      ...
      if !query_result_list.empty?
        ...
        @logger.info("#{LOG_KEY}: query result is NOT empty, size = #{query_result_list.size}")
  ...
  def enqueue_events(query_result_list, queue)
    ...
    # LogDate to @last_read_log_date and .sfdc_info
    ...
    # TODO: might have to move this to the end of the method, in case of a crash in between.
  ...
  def stall_schedule(next_schedule_time)
    ...
    if current_time > next_schedule_time
      ...
      next_schedule_time += @interval while current_time > next_schedule_time
  ...
  def load_last_logged_date
    ...
    if File.exist?(@path)
      ...
      # Note in create_event_list(), which is called in run(), is where .sfdc_info is created or overwritten, so no
